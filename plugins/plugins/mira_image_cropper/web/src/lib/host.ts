 ...
 *   - 选中项：解析 ?media= query（与宿主 preload 同一格式）
...
const host = (typeof window !== 'undefined' && (window.mira || window.eagle)) || {
  ...
  item: { getSelected: async () => parseMediaQuery() },
  log: undefined,
}
...
export async function getSelectedItems(): Promise<MediaInput[]> {
  try {
    return ((await host.item?.getSelected?.()) as MediaInput[]) || []
  } catch (error) {
    logError('[image-cropper] getSelected failed:', error)
    return []
  }
}
