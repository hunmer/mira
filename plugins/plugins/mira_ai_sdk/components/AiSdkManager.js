(function () {
  ...
  var AiSdkManager = {
    ...
    template: [
      ...
      '  <div v-if="!providers.length && !loading" class="flex h-40 items-center justify-center rounded-md border border-dashed text-xs text-muted-foreground">',
      '    还没有服务商，点击右上角「新建服务商」开始配置',
      '  </div>',
      ...
      '  <MiraCard v-if="providers.length" size="sm">',
      '    <MiraCardHeader class="border-b"><MiraCardTitle>聊天测试</MiraCardTitle></MiraCardHeader>',
      '    <MiraCardContent class="space-y-3">',
      ...
      '          </MiraSelect>',
      '        </div>',
      '        <MiraButton class="ml-auto" :disabled="sending || !chatModel || !chatInput.trim()" @click="sendChat">{{ sending ? "回复中..." : "发送" }}</MiraButton>',
      '      </div>',
      '      <div class="rounded-md border">',
      '        <MiraScrollArea class="h-72">',
      '          <div class="space-y-3 p-3">',
      '            <div v-if="!chatMessages.length" class="flex h-56 items-center justify-center text-xs text-muted-foreground">输入消息测试聊天接口（流式）</div>',
      ...
      '        </div>',
      '        <div class="space-y-1"><MiraLabel for="ai-sdk-models">模型列表（每行一个）</MiraLabel>',
      '          <MiraTextarea id="ai-sdk-models" v-model="form.modelsText" rows="5" class="min-h-28 resize-y font-mono" placeholder="deepseek-chat&#10;deepseek-reasoner" />',
      '        </div>',
    ...
    data: function () {
      return {
        ...
        saving: false,
        formError: '',
        form: { name: '', baseUrl: '', apiKey: '', modelsText: '' },
        chatProviderId: '',
        chatModel: '',
        chatInput: '',
        chatMessages: [],
    ...
    methods: {
      ...
      openCreate: function () {
        this.editingId = '';
        this.formError = '';
        this.form = { name: '', baseUrl: '', apiKey: '', modelsText: '' };
      ...
      sendChat: async function () {
        ...
        var reply = { role: 'assistant', content: '', cursor: true };
        ...
        try {
          ...
          while (true) {
            ...
            if (chunk.done) break;
            reply.content += decoder.decode(chunk.value, { stream: true });
          }
        } catch (error) {
