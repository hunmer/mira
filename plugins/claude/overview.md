...
`enabled` 以 server 运行时 `packages/mira-app-server/src/plugins/plugins.json` 为准(上表为源码侧默认印象,实际随安装变化)。`+web` 表示含 `web/` 子目录(客户端动态加载的预览 UI,带 `plugin.json`)。
...
| 文件 | 说明 |
|------|------|
| `plugins/plugins/plugins.recommend.json` | 源码侧推荐插件清单(12 条,不含 mira_gallery_dl / mira_tiptap_format) |
| `packages/mira-app-server/src/plugins/plugins.json` | server 运行时安装状态(name/enabled/path/version/installedAt) |
