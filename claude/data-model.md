...
| 实体 | 说明 | 来源 |
|------|------|------|
| Library | 素材库,每个库独立 SQLite | core storage |
| File | 媒体文件(图片/视频/音频/文档) | core storage |
| Folder | 文件夹层级 | core storage |
| Tag | 文件标签 | core storage |
| User | 用户账号(super/admin/user 权限) | server(内置,原 mira_user 插件已合并) |
| Settings | 应用/服务器设置 | server SettingsManager |
...
Pinia Store(11 个),仅存在于渲染进程内存,持久化由各 Store 自行决定:

AuthStore、LibraryStore、MediaStore、SettingsStore、PluginStore、FolderStore、TagStore、ServerListStore、UploadHistoryStore、AppStateStore、DashboardStore。详见 `packages/mira-client/claude/public-interfaces.md`。
