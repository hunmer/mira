/**
 * 搜索窗口专用入口文件 (Vue版本)
 * 基于通用 FloatingWindowCore 与主进程通信，仅保留搜索业务逻辑。
 */

// 与主进程通信的 bridge（在 initSearchWindow 中创建）
  ...
  try {
    ...
    const app = createApp({
      ...
      mounted() {
        ...
        // 通过通用脚手架建立与主进程的 MessagePort 通信
        const Core = window.FloatingWindowCore
        searchBridge = Core.createBridge({
          role: 'search',
          onMessage: (data) => this.handleBridgeMessage(data),
          onReady: () => {
            // 通知主进程搜索窗口已准备就绪
            searchBridge.send({ type: 'search-window-ready', timestamp: Date.now() })
          },
        })
        searchBridge.start()
      ...
      methods: {
        ...
        handleBridgeMessage(data) {
          if (data && data.type === 'search-results') {
            this.isSearching = false
            this.allResults = data.results || []
            this.currentPage = 1
            this.selectedIndex = -1
          } else if (data && data.type === 'search-error') {
            this.isSearching = false
            this.selectedIndex = -1
            console.error('❌ [SearchWindow] 搜索错误:', data.error)
          }
          // theme-update 已由 FloatingWindowCore 统一处理
        },
        handleDragStart(event) {
          // 通过 bridge 发送拖拽开始请求
          if (searchBridge) {
            searchBridge.requestDrag()
          }
        },
        ...
        sendSearchRequest(keyword, type) {
          ...
          if (searchBridge) {
            ...
            searchBridge.send(searchRequest)
          } else {
        ...
        applyTheme(isDark) {
          this.isDark = isDark
          // 主题 class 由 FloatingWindowCore 统一管理，这里仅同步状态
        },
        ...
        hideSearchWindow() {
          // 通过 bridge 请求关闭搜索窗口（使用通用 close-window 类型）
          if (searchBridge) {
            searchBridge.requestClose()
          }
        },
