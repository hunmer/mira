...
2. **插件系统** — 客户端插件加载、沙箱执行(经 `online_client_plugins/` 索引);插件窗口可申请**受控执行**白名单命令(PluginExecHandlers:ffmpeg/ffprobe/scenedetect)+ plugin-fs 文件原语(为视频剪辑器类插件服务)
3. **Tab 导航** — 基于视图的标签页系统(TabRegistry + TabTypes);含 Webview 标签页(WebviewTabView + FaviconCacheService)
...
7. **多窗口** — 主窗口、悬浮球窗口、通知窗口、搜索窗口、截图窗口
8. **Home 仪表盘** — grid-layout-plus 拖拽卡片布局(dashboardLayout store + CardRegistry + tabs/dashboard/cards/ 卡片体系)
9. **远程 UI 测试** — `renderer/procm-ui-tests/`(约 30 个真实页面用例),经 procm-mcp WebSocket 由 `pnpm run test:ui:remote <name>` 驱动,仅开发构建暴露

## 多进程 / 多窗口架构

| 进程/窗口 | 目录 | 职责 |
|-----------|------|------|
| 主进程 | `src/main/` | MiraApplication 单例:窗口管理、IPC(20 个 Handler)、自定义协议 `mira://`、托盘、自动更新、主进程 i18n |
...
| 截图窗口 | `src/screenshot-window/` | 屏幕截图采集(08-20 后新增:main.ts + style.css + `screenshot-window.html`,配合 ScreenshotHandlers 的 captureDisplay/detectVisibleWindows/complete/cancel) |
| 共享 | `src/shared/` `src/types/` | 跨进程类型 |
| UI 库 | `src/components/ui/` | shadcn-vue 基础组件库(53 目录) |
| 工具 | `src/lib/` | `cn()` 等通用工具 |
...
| UI 库 | **shadcn-vue**(style `new-york`,baseColor `neutral`,cssVariables,icon `lucide`),53 个组件目录/223 个 .vue(2026-08-05 为 34,08-20 为 52,此后 +chart) |
