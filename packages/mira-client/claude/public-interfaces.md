...
| Store | 文件 | 说明 |
|-------|------|------|
| AuthStore | `stores/auth.ts` | 用户认证和会话管理 |
| LibraryStore | `stores/library.ts` | 媒体库管理 |
| MediaStore | `stores/media.ts` | 媒体文件核心状态 |
| SettingsStore | `stores/settings.ts` | 应用配置和服务器设置 |
| PluginStore | `stores/plugin.ts` | 插件市场和管理 |
| FolderStore | `stores/folder.ts` | 文件夹管理 |
| TagStore | `stores/tag.ts` | 标签管理 |
| ServerListStore | `stores/serverList.ts` | 服务器列表 |
...
| SettingsView | `views/SettingsView.vue` | 设置页面 |
| LoginView | `views/LoginView.vue` | 登录页面 |
