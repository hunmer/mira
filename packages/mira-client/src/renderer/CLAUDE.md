...
├── api/                       # API 接口
│   ├── MiraAPI.ts            # Mira API 封装 (603 行)
│   └── TabRegistryAPI.ts     # Tab 注册 API (204 行)
├── components/               # UI 组件
│   ├── business/             # 业务组件 (30+ 个)
│   ├── common/               # 通用组件
│   ├── layout/               # 布局组件
...
│   └── tabs/                 # Tab 视图组件
├── composables/               # 组合式 API
│   ├── useTabs.ts            # Tab 管理 (809 行)
│   ├── TabRegistry.ts        # Tab 注册系统 (156 行)
│   ├── TabTypes.ts           # Tab 类型基类 (114 行)
│   ├── TabSystem.ts          # Tab 系统核心 (149 行)
│   ├── TabHistory.ts         # Tab 历史记录 (190 行)
│   ├── TabPersistence.ts     # Tab 持久化 (176 行)
│   └── tabs/                 # 7 种 Tab 类型定义
├── config/
│   └── defaultShortcuts.ts   # 默认快捷键 (704 行)
├── controllers/               # 业务控制器
│   ├── HomeController/       # 首页控制器 (6 个子模块)
...
│   ├── InitializationService.ts # 初始化 (511 行)
│   ├── MiraSDKService.ts     # SDK 集成 (891 行)
│   ├── PluginService.ts      # 插件服务 (534 行)
│   ├── SearchHandlers.ts     # 搜索处理 (599 行)
│   ├── ShortcutService.ts    # 快捷键 (612 行)
│   ├── WebSocketService.ts   # WebSocket (257 行)
│   ├── MenuService.ts        # 菜单服务 (374 行)
│   ├── ElectronService.ts    # Electron 环境 (256 行)
│   └── search/               # 搜索子服务
├── stores/                    # Pinia 状态管理 (10 个 Store)
├── types/                     # 类型定义
├── utils/                     # 工具函数
│   ├── helpers.ts            # 通用工具 (389 行)
│   ├── fileUtils.ts          # 文件工具 (341 行)
│   ├── ConfigStorage.ts      # 配置存储 (263 行)
│   ├── LibraryStorage.ts     # 库存储 (142 行)
│   ├── DataMigration.ts      # 数据迁移 (110 行)
│   └── errorHandler.ts       # 错误处理 (142 行)
└── views/                     # 页面视图
    ├── HomeView/             # 首页模块 (6 个 composable)
    ├── SettingsView.vue
    ├── LoginView.vue
    ├── FileUploadView.vue
    ├── FilePreviewView.vue
...
10 个 Pinia Store，全部支持持久化到 localStorage:

| Store | 行数 | 描述 |
|-------|------|------|
| `media.ts` | 1100 | 媒体文件核心状态 |
| `auth.ts` | 808 | 用户认证和会话管理 |
| `plugin.ts` | 899 | 插件市场和管理 |
| `settings.ts` | 595 | 应用配置和服务器设置 |
| `serverList.ts` | 397 | 服务器列表 |
| `tag.ts` | 387 | 标签管理 |
| `folder.ts` | 298 | 文件夹管理 |
| `uploadHistory.ts` | 223 | 上传历史 |
...
- `mira-server-sdk`: 后端通信
- PrimeVue + Tailwind CSS: UI 框架
- `v-viewer` / `vue3-lazyload` / `vue-waterfall-plugin-next`: 图片/媒体
- `plyr`: 视频播放器
