...
const records = computed(() => history.uploadRecords)
  ...
  <Dialog v-model:open="open">
    <DialogContent class="w-[min(720px,calc(100vw-2rem))] max-w-none">
      ...
      <div class="max-h-[60vh] overflow-auto">
        <div v-if="records.length === 0" class="py-12 text-center text-muted-foreground">暂无上传记录</div>
        <div v-else class="divide-y divide-border">
          <div v-for="record in records" :key="record.id" class="flex items-center gap-3 py-3">
            ...
            <span :class="record.status === 'success' ? 'text-green-600' : 'text-destructive'" class="text-xs">{{ record.status === 'success' ? '成功' : '失败' }}</span>
