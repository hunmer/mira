  ...
  <div class="p-4 space-y-6">
    ...
    <div>
      <div class="flex flex-wrap items-end gap-4 py-3">
        <div class="flex flex-col min-w-40 flex-1">
          <label class="text-foreground dark:text-muted-foreground text-base font-medium leading-normal pb-2">{{
            t('settings.language') }}</label>
          <Select :model-value="settingsStore.settings.language"
            @update:model-value="(value: any) => handleSettingChange('language', value)">
            <SelectTrigger class="w-full">
              <SelectValue :placeholder="t('settings.selectLanguage')" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem v-for="opt in languageOptions" :key="opt.value" :value="opt.value">{{ opt.label }}
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
        <div class="flex flex-col min-w-40 flex-1">
          <label class="text-foreground dark:text-muted-foreground text-base font-medium leading-normal pb-2">{{
            t('settings.openWebAction') }}</label>
          <Select :model-value="settingsStore.settings.openWebAction"
            @update:model-value="(value: any) => handleSettingChange('openWebAction', value)">
            <SelectTrigger class="w-full">
              <SelectValue :placeholder="t('settings.openWebActionSystem')" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem v-for="opt in openWebActionOptions" :key="opt.value" :value="opt.value">{{ opt.label }}
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
      ...
      <div class="flex gap-3">
        <button v-for="opt in themeOptions" :key="opt.value" type="button"
          ...
          <span :class="[
            ...
            <svg aria-hidden="true" class="size-full" fill="none" viewBox="0 0 88 70" xmlns="http://www.w3.org/2000/svg"
              v-html="opt.preview" />
          </span>
          <span :class="[
    ...
    <div>
      ...
      <div class="flex items-center gap-2">
        <button v-for="color in DEFAULT_PRIMARY_COLORS" :key="color" type="button" :style="{ backgroundColor: color }"
          :class="[
        ...
        <button type="button"
          :style="isCustomPrimaryColor ? { backgroundColor: settingsStore.settings.primaryColor } : undefined" :class="[
          ...
          <input ref="customColorInput" type="color" :value="settingsStore.settings.primaryColor || '#1456f0'"
    ...
    <div>
      ...
      <Select :model-value="settingsStore.settings.themeStyle"
      ...
      <Textarea v-if="settingsStore.settings.themeStyle === 'custom'"
        ...
        class="mt-2 font-mono text-xs min-h-40" :placeholder="t('settings.customCssPlaceholder')" />
      <Button v-if="settingsStore.settings.themeStyle === 'custom'" variant="link" size="sm"
        class="mt-1.5 h-auto p-0 text-xs" @click="openTweakcn">
    ...
    <div>
      ...
      <div class="flex items-center gap-3">
        ...
        <Slider :model-value="[settingsStore.settings.fontSizeScale]" :min="0.85" :max="1.3" :step="0.05" class="flex-1"
    ...
    <div>
      ...
      <div class="flex items-center gap-3">
        ...
        <Slider :model-value="[settingsStore.settings.uiZoom]" :min="0.7" :max="1.5" :step="0.05" class="flex-1"
...
const openWebActionOptions = [
  { label: t('settings.openWebActionSystem'), value: 'system' as const },
  { label: t('settings.openWebActionTab'), value: 'tab' as const },
  { label: t('settings.openWebActionWindow'), value: 'window' as const },
]
