<template>
  ...
  >
    ...
    <div
      ...
      class="selection-box"
      :class="{ 'selection-box--subtract': altMode }"
      :style="selectionBoxStyle"
    />

    <!-- 插槽内容:子元素带 data-selectable-id 即可被框选 -->
...
/**
 * 框选组件(自 mira-client 封装为独立包)。
 *
 * 协议:容器内任何带 [data-selectable-id] 属性的元素都是可选项。
 * 交互:空白处拖拽框选(实时)、Alt+拖拽减选、卡片点击经 handleItemClick
 * 支持 Ctrl 加选 / Shift 范围选 / Alt 单独减选,边缘自动滚动,双击/Escape 清空。
 *
 * 样式脱离 Tailwind 运行时:颜色经 CSS 变量定制 --
 *   --selection-box-color(默认 indigo)与 --selection-box-destructive-color(减选,默认红色)。
 */
import { ref, computed, onMounted, onUnmounted, watch } from 'vue'
import type { SelectionBoxProps, SelectionBoxEmits, SelectableRect } from './types'
...
<style scoped>
/* 等价于原 Tailwind 的 relative w-full h-full select-none pointer-events-auto
   [&>*]:pointer-events-auto(脱离 Tailwind 运行时) */
.selection-container {
  position: relative;
  width: 100%;
  height: 100%;
...
/* 拖拽选择框:颜色经 CSS 变量定制,默认 indigo/红 */
.selection-box {
  position: absolute;
  pointer-events: none;
  z-index: 1000;
  border: 2px dashed var(--selection-box-color, #6366f1);
  background: color-mix(in srgb, var(--selection-box-color, #6366f1) 12%, transparent);
  backdrop-filter: blur(2px);
  border-radius: 4px;
  animation: selection-box-pulse 2s infinite;
}
.selection-box--subtract {
  --selection-box-color: var(--selection-box-destructive-color, #ef4444);
}

@keyframes selection-box-pulse {
  0%, 100% {
    opacity: 0.6;
    transform: scale(1);
  }
