 ...
 *
 * 所有解析都带调试输出(经 dbg),便于在扩展 console 用 `[mira-ext][drag]` 过滤定位。
 */
...
export function parseDrop(e: DragEvent): ParsedDrop {
  ...
  if (!dt) {
    dbg.log('drag', 'parseDrop: no dataTransfer');
    return { files: [], urls: [], hasContent: false };
  }
...
/**
 * 推断链接资源类型(用于 UPLOAD_FROM_URL 的 kind)。
