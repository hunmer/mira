...
/**
 * SafariBrowser —— 一个纯 SVG 绘制的 Safari 浏览器外框 mockup。
 *
 * 从 21st.dev bundle 还原：https://cdn.21st.dev/bundled/1111.html
 * 无运行时交互、无额外依赖；图标全部以内联 SVG path 还原，
 * 顶部工具栏图标组用 `mix-blend-luminosity` 还原原组件的明度混合质感。
 *
 * 内容区支持两种用法（二选一）：
 *  1. 传 `src`：渲染一张网页截图（向后兼容旧用法）
 *  2. 传 `children`：在内容区渲染任意 React 组件（如实时 demo、瀑布流等）
 * 若同时传入，`children` 优先。
 *
 * @example
 * // 截图
 * <SafariBrowser src="/screenshot.png" url="your-site.com" />
 * // 实时组件
 * <SafariBrowser url="mira.app"><MyLiveDemo /></SafariBrowser>
 */
...
export type SafariBrowserProps = {
	/** 浏览器内容区显示的网页截图 URL。传了 children 时此项被忽略。 */
	src?: string;
	/** 地址栏显示的文字（默认 "designali.in"，来自原 demo） */
	url?: string;
...
export function SafariBrowser({
	...
	// 内容区按比例定位到原 <image> 占据的区域（x:1..1201, y:52..741）。
	// 用百分比，让外层 aspect-ratio 容器在任意宽度下都精确对齐 SVG 外框。
	const contentStyle = {
		left: `${(1 / width) * 100}%`,
		right: `${(1 / width) * 100}%`,
		top: `${(52 / height) * 100}%`,
		bottom: `${((height - 741) / height) * 100}%`,
	} as const;
	...
	return (
		...
		>
			...
			>
				...
				<defs>
					<clipPath id="safari-path0">
						<rect width={width} height={height} fill="white" />
					</clipPath>
				</defs>
			</svg>

			{/* 内容区：children 优先，回退到 src 截图 */}
...
/**
 * 默认 demo 入口 —— 演示内容区直接渲染实时组件（瀑布流），而非静态截图。
 */
export default function SafariBrowserDemo() {
	return (
		<div className="relative w-full max-w-[1203px]">
			<SafariBrowser url="mira.app" className="size-full">
				<div className="size-full overflow-hidden bg-gradient-to-br from-background via-background to-muted">
					...
					<div className="flex items-center gap-2 border-b border-border/60 px-4 py-2">
						...
						<span className="text-xs font-medium text-foreground">Mira</span>
