 ...
 *  - hover 上浮 + 阴影、图片缓慢放大；点击切换选中态（描边 + 勾选圆点）
 *  - 底部玻璃信息浮层（文件名 / 大小 / 文件夹 / 标签 badge）
 *  - motion stagger 入场
 *
 * 自带一份虚拟数据 `mediaWaterfallMockData`，可直接 `<MediaWaterfall />` 使用。
 *
 * @example
 * <MediaWaterfall />
 * <MediaWaterfall items={myItems} className="columns-3 [column-gap:1.25rem]" />
 */
...
function MediaWaterfallCard({ item, selected, onToggle }: CardProps) {
	return (
		<div
			role="button"
			tabIndex={0}
			...
			>
				...
				<div
					className={cn(
						...
						selected ? "bg-primary/90" : "bg-white/70 dark:bg-muted/70",
					)}
				>
					<h3
						className={cn(
