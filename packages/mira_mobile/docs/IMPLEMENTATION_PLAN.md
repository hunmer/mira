...
- **两套 SDK 并存**：本次只用 `lib/mira_sdk/`（已测通、与后端契约对齐，43 测试全绿）；`lib/src/packages/mira_sdk/` 是旧的、不用、可忽略（不在本 plan 范围删除，避免牵连）。
...
### 第 3 步：主页 + 文件夹/标签列表（多选过滤）
- **`main_shell_screen.dart`**：保留三 Tab。Tab1=图片网格，Tab2=文件夹/标签树，Tab3=设置。
- **文件夹/标签 Tab**（改造 `tree_view_screen.dart`）：用 `folder_provider`/`tag_provider` 拉真实列表（`getAll`），客户端组树（根=parentId==null）。**多选模式**：每个节点带 checkbox，选中写入 `file_filter_provider`。同时提供"全部/未分类/未标签"基础分类（对应 `FileFilters(uncategorized:true)` / `FileFilters(untagged:true)` / 默认）。选中变化→Tab1 网格自动刷新（Riverpod watch）。
